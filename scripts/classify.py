...
def create_no_valid_list():
    if not cfg.not_valid_path.exists():
        with open(cfg.not_valid_path, "w") as f:
            json.dump([], f)


def create_not_full_game():
    if not cfg.not_full_path.exists():
        with open(cfg.not_full_path, "w") as f:
            json.dump([], f)


def create_backup_file():
    if not cfg.backup_files.exists():
...
def classify_games(add_id: list[int], game_name: list[str]) -> list[int]:
    ...
    with open(cfg.not_valid_path, "r") as f:
    ...
    while len(apps) > 0:
        ...
        try:
            if item_type != "game":
                app = (
                    app[0],
                    data[str(app[0])]["data"]["name"],
                    int(data[str(app[0])]["data"]["fullgame"]["appid"]),
                )
        except Exception:
